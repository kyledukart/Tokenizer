...
class Evaluator {
 public:
  Evaluator( const string &s ) : str( s ) { // constructor receives an exp
    ...
    PREC_TABLE[6].inputSymbol = 21;     // UN_MINUS
    PREC_TABLE[6].topOfStack = 22;      //
    PREC_TABLE[7].inputSymbol = 21;     // UN_PLUS
    ...
    PREC_TABLE[23].inputSymbol = 5;    // BITOR
    PREC_TABLE[23].topOfStack =  6;

    PREC_TABLE[24].inputSymbol = 3;    // AND
    PREC_TABLE[24].topOfStack =  4;

    PREC_TABLE[25].inputSymbol = 2;    // OR
    PREC_TABLE[25].topOfStack =  3;

    PREC_TABLE[26].inputSymbol = 0;    // VARA
    PREC_TABLE[26].topOfStack =  0;

    PREC_TABLE[27].inputSymbol = 0;    // VARB
    PREC_TABLE[27].topOfStack =  0;

    PREC_TABLE[28].inputSymbol = 0;    // VARC
    PREC_TABLE[28].topOfStack =  0;

    PREC_TABLE[29].inputSymbol = 1;    // ASSIGNA
    PREC_TABLE[29].topOfStack =  1;

    PREC_TABLE[30].inputSymbol = 1;    // ASSIGNB
    PREC_TABLE[30].topOfStack =  1;

    PREC_TABLE[31].inputSymbol = 1;    // ASSIGNC
    PREC_TABLE[31].topOfStack =  1;
  }

  NumericType getValue( );                  // Do the evaluation

 private:
  Precedence PREC_TABLE[32];                // precedence table
